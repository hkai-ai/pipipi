...
} from "../src/process-runs/retention/index.js";
import type { PostgresRetentionCleanup } from "../src/process-runs/retention/postgres.js";
    ...
    it("uses one cutoff across bounded batches and returns a resumable cursor", async () => {
        const cleanupBatch = vi
            ...
            .mockResolvedValueOnce(
                batch({ examined: 2, nextCursor: RUN_IDS[0] }),
            )
            .mockResolvedValueOnce(
                batch({ examined: 2, nextCursor: RUN_IDS[1] }),
            )
