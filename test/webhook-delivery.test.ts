...
} from "../src/webhooks/delivery/target-policy.js";
...
describe("Webhook Delivery", () => {
    it("signs the exact raw body and Standard Webhooks metadata", () => {
        const payload =
            '{"type":"process_run.succeeded","data":{"runId":"run-1"}}';
    ...
    it("claims, sends, and persists a successful Delivery without exposing transport to the Process", async () => {
        const logs: unknown[] = [];
        const complete = vi.fn<WebhookDeliveryStore["complete"]>(
            async () => true,
        );
    ...
    it("retries only network, 429, and 5xx failures with bounded backoff", async () => {
        const reschedule = vi.fn<WebhookDeliveryStore["reschedule"]>(
            async () => true,
        );
        const complete = vi.fn<WebhookDeliveryStore["complete"]>(
            async () => true,
