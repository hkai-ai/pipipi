...
describe("controlled MVP HTTP boundary", () => {
  ...
  it("returns a safe error when the process executor rejects unexpectedly", async () => {
    ...
    const service = await startRequestListener(
      createProcessingRequestListener({
        execute: async () => {
          executionCalls += 1;
          if (executionCalls === 1) throw new Error(sensitiveFailure);
          return {
  ...
  it("emits one allowlisted completion record for a successful process run", async () => {
    ...
    expect(JSON.stringify(records)).not.toContain("sensitive business input");
    expect(JSON.stringify(records)).not.toContain("sensitive business output");
  });
  ...
  it("does not spend execution capacity on health or transport rejections", async () => {
    ...
    const validResponse = await executeContent(service.url, "still accepted");
      ...
      agentRuntime: {
        optimize: async (request) => {
          ...
          await new Promise((resolve) =>
            setTimeout(resolve, request.content.endsWith("alpha") ? 15 : 5),
          );
...
function executeContent(serviceUrl: string, content: string): Promise<Response> {
