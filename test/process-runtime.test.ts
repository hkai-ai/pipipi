...
import type { CompletedProcessRun } from "../src/process-runtime/records.js";
...
describe("Process Runtime", () => {
    it("captures and validates a server-owned retry policy", () => {
        ...
        expect(Object.isFrozen(process.retryPolicy.retryableErrorCodes)).toBe(
            true,
        );
    ...
    it("builds an immutable catalog with exact Business Process versions", async () => {
        ...
        expect(registry.find({ id: "test-processing", version: "v1" })).toBe(
            v1,
        );
        expect(registry.find({ id: "test-processing", version: "v2" })).toBe(
            v2,
        );
        expect(
...
function createTestProcessingRegistration(options: {
    ...
    return defineProcessRegistration({
        ...
        execute: async (input, context) => {
            ...
            const processed = await options.textProcessing.process(
                input.value,
                {
