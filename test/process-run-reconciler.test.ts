...
describe("Process Run Reconciler", () => {
  ...
  it("reports a dry-run without mutating the queue or outbox", async () => {
    ...
    const reconciler = createProcessRunReconciler({
      ...
      queue: fakeQueue({
        enqueue,
        inspectJobs: async () => [{ runId: runId(5), state: "missing" }],
      }),
      clock: () => AS_OF,
      createRecoveryId: () => RECOVERY_ID,
    });
  ...
  it("isolates one queue failure and audits the remaining batch", async () => {
    ...
    const reconciler = createProcessRunReconciler({
      ...
      queue: fakeQueue({
        ...
        enqueue: async (job) => {
          if (job.runId === runId(6)) throw new Error("Redis unavailable");
          return "enqueued";
        },
      }),
      clock: () => AS_OF,
    ...
    expect(recordRecoveryItem).toHaveBeenCalledWith(
      expect.objectContaining({
        item: expect.objectContaining({ runId: runId(6), action: "failed" }),
      }),
    );
  });
  ...
  it("records a partial repair when enqueue succeeds before Outbox acknowledgement fails", async () => {
    ...
    const reconciler = createProcessRunReconciler({
      ...
      queue: fakeQueue({
        inspectJobs: async () => [{ runId: runId(10), state: "missing" }],
      }),
      clock: () => AS_OF,
      createRecoveryId: () => RECOVERY_ID,
    });
  ...
  it("uses the same audited stale path for periodic reconciliation", async () => {
    ...
    const reconciler = createProcessRunReconciler({
      ...
      queue: fakeQueue({
        inspectJobs: async () => [{ runId: runId(9), state: "runnable" }],
      }),
      queuedAgeMs: 60_000,
      clock: () => AS_OF,
      createRecoveryId: () => RECOVERY_ID,
  ...
  it("rejects unsafe bounds and actors before starting", async () => {
    ...
    ).rejects.toThrow("Process Recovery actor must be 1 to 512 UTF-8 bytes");
  });
});
...
function fakeQueue(
  ...
  return {
    ...
    inspectJobs: async (runIds) =>
      runIds.map((runIdValue) => ({ runId: runIdValue, state: "missing" })),
