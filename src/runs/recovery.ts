...
export function createProcessRunReconciler(options: {
  ...
  const recover: ProcessRunReconciler["recover"] = async (request) => {
    ...
    if (request.cursor) assertUuid(request.cursor, "Process Recovery cursor");
    ...
    try {
      ...
      const inspections =
        selection.candidates.length === 0
          ...
          : await options.queue.inspectJobs(
              selection.candidates.map((candidate) => candidate.runId),
            );
      ...
      for (const candidate of selection.candidates) {
        counters.found += 1;
        if (candidate.reason === "active_lease") counters.activeLeases += 1;
        if (candidate.pendingOutbox) counters.pendingOutbox += 1;
        const queueState = inspectionByRunId.get(candidate.runId);
        if (!queueState) throw new Error("Queue inspection omitted a Process Run");
        ...
        try {
          item = request.dryRun
            ...
            : await repairItem(options, candidate, queueState, clock, counters);
        } catch {
          counters.failed += 1;
          item = Object.freeze({
            ...
            queueState,
            action: "failed",
            outboxAction: candidate.pendingOutbox ? "failed" : "none",
          });
        }
        items.push(item);
...
function dryRunItem(
  ...
  return Object.freeze({
    ...
    action,
    outboxAction:
      candidate.pendingOutbox && (validQueueJob || action === "would_enqueue")
        ? "would_acknowledge"
        : "none",
  });
}
...
function validatedInspections(
  ...
  for (const inspection of inspections) {
    if (!expected.has(inspection.runId) || result.has(inspection.runId)) {
      throw new Error("Queue inspection returned unexpected Process Runs");
    }
