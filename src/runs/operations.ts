...
export type PostgresAsyncOperations = Readonly<{
  snapshot: (request?: { asOf?: string }) => Promise<PostgresAsyncOperationsSnapshot>;
  ready: () => Promise<void>;
}>;
...
export function createPostgresAsyncOperations(options: {
  ...
  return Object.freeze({
    snapshot: async (request = {}) => {
      ...
      const recentSince = new Date(measuredAtMs - recentWindowMs).toISOString();
      const stuckBefore = new Date(measuredAtMs - stuckRunAgeMs).toISOString();
      const result = await options.pool.query<OperationsRow>(
        operationsQuery,
        [measuredAt, recentSince, stuckBefore],
      );
      const row = result.rows[0];
      if (!row) throw new Error("Async operations snapshot is unavailable");

      const succeededRecent = count(row.succeeded_recent, "recent succeeded Runs");
      ...
      const cleanupCompletedAt = optionalTimestamp(row.cleanup_completed_at);
      const recoveryCompletedAt = optionalTimestamp(row.recovery_completed_at);
      ...
      return Object.freeze({
        ...
        runs: Object.freeze({
          ...
          succeededRecent,
          failedRecent,
          failureRateRecent: rate(failedRecent, succeededRecent + failedRecent),
          oldestQueuedAgeMs: duration(row.oldest_queued_age_ms, "oldest queued Run age"),
          queueWaitP95Ms: duration(row.queue_wait_p95_ms, "queue wait p95"),
          executionP95Ms: duration(row.execution_p95_ms, "execution p95"),
          stuck: count(row.stuck_runs, "stuck Runs"),
        }),
        outbox: Object.freeze({
          processPending: count(row.process_outbox_pending, "Process Outbox pending"),
          webhookPending: count(row.webhook_outbox_pending, "Webhook Outbox pending"),
        ...
        webhooks: Object.freeze({
          ...
          failureRateRecent: rate(
            ...
            webhookSucceededRecent + webhookFailedRecent + webhookExhaustedRecent,
          ),
        ...
        cleanup: Object.freeze({
          ...(cleanupCompletedAt ? { lastCompletedAt: cleanupCompletedAt } : {}),
          lastDeferredRuns: count(row.cleanup_deferred_runs, "deferred cleanup Runs"),
        }),
        recovery: Object.freeze({
          ...(recoveryCompletedAt ? { lastCompletedAt: recoveryCompletedAt } : {}),
          ...(recoveryStatus ? { lastStatus: recoveryStatus } : {}),
          lastMissingJobs: count(row.recovery_missing_jobs, "recovery missing jobs"),
          lastFailedItems: count(row.recovery_failed_items, "recovery failed items"),
        }),
        storage: Object.freeze({
          databaseBytes: bytes(row.database_bytes, "database size"),
          asyncTablesBytes: bytes(row.async_tables_bytes, "async table size"),
        }),
      });
    },
    ready: async () => {
...
export function createPostgresAsyncReleaseReadiness(options: {
  ...
  return async () => {
    ...
    const result = await options.pool.query<ReleaseGateRow>(
      ...
      [
        asOf,
        new Date(asOfMs - stuckRunAgeMs).toISOString(),
      ],
    );
    const row = result.rows[0];
...
function normalizedTimestamp(value: string): string {
  ...
  if (!Number.isFinite(time)) throw new Error("Async operations timestamp is invalid");
  return new Date(time).toISOString();
}

function optionalTimestamp(value: Date | string | null | undefined): string | undefined {
  return value === null || value === undefined
    ? undefined
    : normalizedTimestamp(value instanceof Date ? value.toISOString() : value);
}
...
function count(value: number | string | null | undefined, label: string): number {
...
function duration(value: number | string | null | undefined, label: string): number {
  const parsed = Number(value ?? 0);
