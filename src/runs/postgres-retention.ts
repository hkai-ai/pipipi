    ...
    return Object.freeze({
        cleanupBatch: async (request) => {
          ...
          `,
                ...
                if (!lastRunId)
                    throw new Error("Retention cleanup candidate is missing");
          ...
          `,
                ...
                const metadataDueCount = Number(
                    metadataDue.rows[0]?.count ?? "0",
                );
        ...
        `,
            ...
            if (
                result.rows[0]?.cleanup_batches !== "retention_cleanup_batches"
            ) {
                throw new Error(
                    "Retention cleanup database migration is not ready",
                );
            }
        },
    });
