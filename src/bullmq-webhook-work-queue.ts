...
export function createBullMqWebhookWorker(options: {
  ...
  >(
    ...
    {
      connection: { url: parseRedisUrl(options.redisUrl), maxRetriesPerRequest: null },
      prefix,
      autorun: false,
      concurrency,
    ...
    close: async () => {
      ...
      try {
        if (runPromise) {
          ...
          if (forced) worker.cancelAllJobs("Webhook Worker shutdown grace expired");
        }
      } finally {
...
function parseRedisUrl(value: string): string {
  ...
  try {
    ...
    throw new Error("Webhook Redis URL must be a valid redis:// or rediss:// URL");
  }
  if (
