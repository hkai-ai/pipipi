...
export class PiTemplateAgent implements TemplateAgent {
    ...
    constructor(options: Omit<PiStructuredAgentOptions, "instructions">) {
        ...
        const common = [
            ...
            "输出紧凑 JSON，证据通常 10–35 个汉字，最多 96 字符；只记支持当前判定的图像事实或替换结果，禁止复述规则、字段定义和整幅画面。正式 visualContract 仍完整保留事实，不受证据字数约束。先确定模板接管的特征再设计推荐项；身份未识别则用简洁可见描述，不猜专名。metadata 仅含 tags。",
        ];
        this.#agent = new PiStructuredAgent({
            ...
            instructions: [
                ...common,
                `analysis 使用紧凑格式：${schemaText(compactAnalysisSchema)}。`,
