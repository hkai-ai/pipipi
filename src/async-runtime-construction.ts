...
} from "./bullmq-process-work-queue.js";
import { createOutboxDispatcher } from "./outbox-dispatcher.js";
import { createPostgresProcessOutbox } from "./postgres-process-outbox.js";
import {
...
export function constructProcessWorkerService(
  ...
  if (claimLeaseMs <= processTimeoutMs) {
    throw new Error("PROCESS_RUN_CLAIM_LEASE_MS must exceed PROCESS_TIMEOUT_MS");
  }
  const retention = {
  ...
  const worker = createBullMqProcessWorker({
    ...
    prefix: queuePrefix,
    concurrency,
    workerName: optionalNonEmptyConfiguration(environment.PROCESS_WORKER_NAME),
    shutdownGraceMs,
    lockDurationMs,
    stalledIntervalMs,
    maxStalledCount,
...
function parsePostgresConnectionString(value: string | undefined): string {
  ...
  if (!candidate) throw new Error("DATABASE_URL is required for this runtime role");
  try {
...
function parseRedisConnectionString(value: string | undefined): string {
  const candidate = value?.trim();
  if (!candidate) throw new Error("REDIS_URL is required for this runtime role");
  try {
    const url = new URL(candidate);
    if (
