...
与原操作环境的差异：审批从 Codex 对话迁至 Memebuy 页面；Python 实现适配为 TypeScript；当前入口是独立单图任务，不提供原 5000 项任务分片或任务级多样性分配。risks 映射为字符串数组，十二段均使用字符串；摘要、审批、revision 和上传事实由服务端持有。并非把整个 Python 批量工具安装到生产 Agent。
