...
若审计失败且不能从脱敏错误分类确定修复方式，可对已上传的候选镜像手动运行 `Production database boundary inspection`。该只读 workflow 绑定 `production-database-inspection` 受保护 Environment，输入活动 revision 与候选 SHA；它只报告当前连接是否使用 TLS、是否切换角色、是否拥有管理权限或其他数据库访问，以及固定 CA 后 TLS 和有效角色直登是否可用。artifact 不包含数据库名、角色名、主机、连接串或原始错误。检查入口不执行 SQL DDL/DCL、不改 `.env`、不重启容器；修复仍需另行评审和批准。
