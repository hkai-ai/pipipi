...
本文面向开发和发布 `crt-interface-image/v1` 的工程师，定义来源 URL 摘要、GPT Image 2 原始结果、CRT 最终结果和调用元数据如何按环境保留。证据策略由服务端配置，并绑定准确的 Business Process 版本；产品调用方不能通过 `/execute` 开启、关闭或选择保存目录。

## 当前结论

仓库内的公网 URL 验收 Implementation 支持 `off`、`metadata` 和 `full` 三档证据策略。`accept:crt-business` 默认使用 `full`，便于开发者复查真实参考图编辑；生产 `POST /crt-images` 必须默认使用 `off`，只有完成数据授权、隔离、期限和删除评审后才能启用其他模式。
...
| `full` | 模型原始图、CRT 成品和 `manifest.json` | 本地开发、视觉调试、显式付费验收 |
...
来源图片不由服务端下载或保留；manifest 只写完整 URL 的 SHA-256。`raw-gpt-image-2.*` 是图片 Adapter 返回、尚未经过 finalizer 的准确字节；`final-crt.png` 是确定性调色板、像素格、扫描线和签名处理后的结果。

## 执行与关联

```mermaid
flowchart LR
    Source["Public source URL"] --> Edit["FAL Adapter<br/>reference edit"]
    ...
    Policy -->|"full"| Pixels["raw/final files"]
...
- 上游返回的 token usage；
- 调色板和画幅；
- 来源 URL 的 SHA-256，以及模型原始图和最终图的媒体类型、尺寸、字节数和 SHA-256；
...
命令完成后先打开 `acceptance/latest.md`。报告会列出本次 Run 的 manifest、模型原始图和最终图。再核对以下事实：

1. manifest 的 `source.urlSha256` 与验收报告的来源 URL 摘要一致。
...
- `full` 必须把模型原始图和最终图视为敏感业务内容；访问、下载和删除都要审计。
...
仓库不会自动清理 `artifacts/` 中的开发证据。维护者应按团队批准的期限删除本地文件，且不得提交模型原始图、最终图或 manifest 中的业务关联信息。
