...
同一 `v1` 兼容五个可选预设：蓝橙穿字、蓝橙斜切、黑红宣言、黑红取景框、黑红斜排。默认搭配与结构化覆盖由 [mono-color.ts](../../../../src/processes/photo-poster/mono-color.ts) 拥有；每种版式明确标题用色、分行、位置与主体穿插关系，默认使用干净近白纸底、阴影细网点和清晰轮廓。显式覆盖后，油墨分工跟随最终版式与配色；用户原文优先于分行，不强制改变大小写或补词。Registration 在编译前传入服务端解析的设计约束，并在最终图片指令中重申，覆盖通用 Skill 的复古印刷要求。原图 URL、用户文字和补充说明不进入文本 Agent。请求没有新增设计参数时保留旧行为，不修改冻结 Runtime Skill 的字节或版本。
