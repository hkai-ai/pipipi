...
| Installed Skill Catalog | [`src/agent-runtime/catalog.ts`](../src/agent-runtime/catalog.ts) | Startup Construction | 已安装 Skill 的准确名称、版本和 SHA-256；启动期完整性校验与精确版本解析 |
...
启动时，Installed Skill Catalog 校验生产组装声明的全部本地 Runtime Skill，并按准确名称和版本向各 Agent 提供绑定。生产请求到达后，production catalog 精确找到 Process Registration；Registration 执行业务定义；需要模型任务时调用窄 Agent Interface；Pi Adapter 只加载已绑定的 Runtime Skill，并只提供代码中显式配置的 Tool；Registration 再校验结果并决定是否调用 Business Capability。Runtime Skill 影响模型如何完成任务，不拥有流程、权限或副作用。
