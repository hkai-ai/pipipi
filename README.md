...
仓库还提供默认不挂载的 Agent Conversations 多轮文本能力。受控测试 Application 可以用准确 Agent id/version 和 `Idempotency-Key` 创建首轮 Turn，用 `afterTurnId` 可靠追加，并按 owner 游标分页查询；服务端保证单活跃 Turn、无分支 sequence，并从成功公共历史重建受预算 Context。当前仍使用内存 Store 与确定性 Queue，不支持图片或生产恢复。完整当前契约见 [Agent Conversations API](docs/api.md#agent-conversations多轮文本)。
