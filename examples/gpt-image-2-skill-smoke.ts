...
if (!posterSkill) throw new Error("The minimal zine poster Skill is unavailable");
...
const report: SmokeReport = {
  ...
  agent: {
    ...
    model: agentModel,
    apiMode: openAIApiMode,
    ...(agentDurationMs === undefined ? {} : { durationMs: agentDurationMs }),
  ...
  image: {
    ...
    ...(imageDurationMs === undefined ? {} : { durationMs: imageDurationMs }),
...
async function compilePosterWithSkill(options: {
  ...
  try {
    ...
    return compiledPosterSchema.parse(parseLastAssistantJson(session.messages));
  } finally {
...
function fallbackPoster(sourceTheme: string, exactText: string): CompiledPoster {
  return {
    prompt:
    ...
    recipe:
      "lower-left-float / torn-paper clipping / short phrase pressed against image edge / cobalt-blue block / halftone degradation / memory",
    interpretation:
...
function evaluateSmoke(options: {
  ...
  return [
    ...
    },
    {
      ...
      passed: /negative space/i.test(prompt) && /(?:7|8|9)\d%/.test(prompt),
    },
    {
...
function parseLastAssistantJson(messages: readonly unknown[]): unknown {
  ...
  if (
    ...
    throw new Error("The poster Agent did not produce a successful response", {
