...
if (!healthResponse.ok) {
    throw new Error(
        `Staging health check failed with HTTP ${healthResponse.status}`,
    );
}
...
if (failureResponse.status !== 400 || !isSafeInvalidInputFailure(failureBody)) {
    throw new Error(
...
async function readJson(response: Response): Promise<unknown> {
    try {
        ...
        throw new Error(
            `Staging service returned non-JSON HTTP ${response.status}`,
        );
    }
}
