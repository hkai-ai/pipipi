            ...
            JSON.stringify(
                {
                    ...
                    next: "Configure OpenAI credentials, then run npm run test:skill-ab without SKILL_AB_DRY_RUN.",
                },
                null,
                2,
            ),
...
function evaluateExperiment(results: {
    ...
    return [
        {
            criterion:
                "direct path calls the Business API once with unchanged input",
            passed:
        ...
        {
            criterion:
                "candidate Skill canary reaches the Business API Tool input",
        ...
        },
        {
            ...
            passed:
                !candidateInput
                    .toLowerCase()
                    .includes("there is no doubt but that") &&
                !candidateInput
                    .toLowerCase()
                    .includes("owing to the fact that"),
        },
        {
        ...
        {
            criterion:
                "each response is exactly the corresponding Business API result",
            passed: [results.direct, results.control, results.candidate].every(
                (result) =>
                    result.businessApiInputs.length === 1 &&
                    result.output ===
                        `Processed: ${result.businessApiInputs[0]}`,
            ),
        },
    ];
}
