...
export class AliyunOssStorage implements ObjectStorageCapability {
    ...
    constructor(
        ...
        if (signedUrlTtlSeconds > 604_800) {
            throw new Error(
                "OSS signed URL TTL must not exceed 604800 seconds",
            );
        }
        if (options.cname && !endpoint) {
            throw new Error(
                "OSS endpoint is required when CNAME mode is enabled",
            );
        }
        ...
        const clientFactory =
            ...
            ((clientOptions) => new OSS(clientOptions));
                ...
                ...(this.#urlAccess === "signed"
                    ? {
                          urlExpiresAt: new Date(
                              this.#now().getTime() +
                                  this.#signedUrlTtlSeconds * 1_000,
                          ).toISOString(),
                      }
                    : {}),
