...
- 原 Skill 的文字排版通过逐区 layoutRefs 引用正式 visualContract，程序投影为 layout；保留和可编辑文字必须覆盖整行形态、基线、字形、间距、对齐层级与位置。独立复核同时引用文字区域与正式约束，不从旧方案补造视觉事实，不把字形歪斜等同于整行弧度。
...
- 模板视觉事实只在正式 visualContract 中编写，模型分析用有界引用定位；语义副本、事实副本和推荐项代入由服务端投影。引用失效进入原有修正预算，不自动拼接冲突句子；自复核仍需审查原图事实、八轴取舍与推荐项特征兼容性。
...
- 模板特征权限与复核报告使用具名对象，部分分析继续使用紧凑行；九轴权限按 replace_identity binding 要求，其他槽位为 null。证据按固定来源要求非空且具体，不新增最少字符门禁；模型证据最多 96 字符，正式视觉事实不受此限。复核上下文直接读取引用计划，引用错误不得吞掉其他有效约束；完整业务校验和公开 Gallery JSON 保持不变。
