...
Agent Conversations Module 已实现可靠多轮文本：调用方用准确 Agent id/version 和 `Idempotency-Key` 创建包含第一轮 Turn 的 Conversation，之后用最后接受的 `afterTurnId` 追加增量 Turn，并按 owner 分页查询 queued、running、succeeded 或 failed 状态。Store 原子保证单活跃 Turn、无分支 sequence 与操作级幂等；Worker 从成功公共历史重建受预算 Context 和 Working Summary。该 Interface 只在 Application 显式注入内存 Store、确定性 Queue、Agent Registry 与可信 caller identity 时挂载；production Composition Root 尚未装配，所以默认服务仍返回 404。图片、PostgreSQL、BullMQ、Tool Ledger、删除和 production `design-assistant/v1` 仍是后续阶段。
